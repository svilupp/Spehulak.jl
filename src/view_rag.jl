function tab_rag_viewer()
    [
            ...
            ## Core Conversation that the user sees
                ...
                key! = R"item.id", [
                    messagecard("{{item.content}}",
                        title = "{{item.title}}"; card_props = Dict(:class => "{{item.class}}")),
        ...
        expansionitem(
            ...
            @iif("rag_show.has_refined_answer && !!rag_show.conv_final_answer"))
    ] |> htmldiv
end